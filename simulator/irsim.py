...
class IRSim(QMainWindow, ui_mainwindow.Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        #self.triggerSignal.emit("triggered()")
        #self.curRowSignal.emit("currentRowChanged(int)")
        #self.triggerSignal.connect(self.fileOpen)
        #self.action_Open.openSignal = pyqtSignal(object)
        #self.action_Open.openSignal.emit("triggered()")
        #self.action_Open.openSignal.connect(self.fileOpen)
        #self.action_Open.clicked.connect(self.openSignal)
        #self.openSignal.connect(self.fileOpen)
    ...
    def fileOpen(self):
        if not self.okToContinue():
            ...
            fname = QFileDialog.getOpenFileName(self, 'IR Simulator - Open IR file', dir, 'IR files (*.ir)')
